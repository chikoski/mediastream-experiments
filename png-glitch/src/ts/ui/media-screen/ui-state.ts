...
function glitchEffect(
    ...
    const glitcher = createGlitchTransformStream(
        canvas,
        (glitcher) => {
            const height = Math.floor(50 * Math.random());
            const devider = Math.floor(canvas.height * 0.5 * Math.random()) + height;

            glitcher.getScanLines().forEach((scanline, index) => {
                if(index % devider < height) {
    ...
    processor.readable.pipeThrough(glitcher).pipeTo(generator.writable);
    return generator
}
            ...
            (async () => {
                if(context != null) {
                    ...
                    for(let i = 0; i < copyTimes; i++) {
                        const src = Math.floor(Math.random() * (canvas.height - 50));
                        const dest = Math.floor(Math.random() * (canvas.height - 50));
                        context.drawImage(canvas, 0, src, canvas.width, 50, 0, dest, canvas.width, 50);
                    }
